...
import {Question, QuestionWithReason} from "../exams/exam";
import {PaginationFilter, PaginationMetaData} from "../../../pkg/types/pagination";

export interface TestRepository {
    getTests: (filter: PaginationFilter) => Promise<{ tests: Test[]; metadata: PaginationMetaData }>
    CreateTest: (test: PartialWithRequired<Test, "questions" | "questionMode" | "userId" | "examId" | "endTime" | "type">) => Promise<{
        testId: string,
        endTime: Date
    }>
    ...
    GetTestQuestions: (filter: PaginationFilter) => Promise<{
